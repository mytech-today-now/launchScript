...
<#
.SYNOPSIS
    Silent installation script for Belarc Advisor
    
.DESCRIPTION
    This script automatically downloads and installs the latest stable version of
    Belarc Advisor from the official website. Belarc Advisor is a system information
    tool that provides detailed information about your computer's hardware and software.
    
.PARAMETER Force
    Force reinstallation even if Belarc Advisor is already installed
...
.NOTES
    ...
    Official Website: https://www.belarc.com/
    Download Page: https://www.belarc.com/products_belarc_advisor
#>
...
$script:AppConfig = @{
    Name = "Belarc Advisor"
    DisplayName = "Belarc Advisor*"
    Publisher = "Belarc, Inc."
    OfficialWebsite = "https://www.belarc.com/"
    DownloadPage = "https://www.belarc.com/products_belarc_advisor"
    DirectDownloadUrl = "https://downloads.belarc.com/en/advisor/advisorinstaller.exe"
...
function Get-BelarcAdvisorLatestVersion {
    <#
    .SYNOPSIS
        Get the latest stable version of Belarc Advisor from the download page
    .OUTPUTS
        Hashtable containing version information and download URLs
    #>
    try {
        Write-AppLog "Fetching latest Belarc Advisor version information..." -Level INFO -Component $script:AppConfig.Name
        ...
        try {
            ...
            if ($content -match 'Version\s+(\d+\.\d+(?:\.\d+)?(?:\.\d+)?)' -or $content -match 'advisor.*?(\d+\.\d+(?:\.\d+)?(?:\.\d+)?)') {
                $version = $matches[1]
            }
            else {
                $version = "Latest"
        ...
        # Belarc Advisor uses a single installer for all architectures
        $downloadUrl = $script:AppConfig.DirectDownloadUrl
        $fileName = "BelarcAdvisorInstaller.exe"
...
function Get-InstalledBelarcAdvisorVersion {
    <#
    .SYNOPSIS
        Get the currently installed version of Belarc Advisor
    .OUTPUTS
    ...
    try {
        ...
        if ($installedPrograms.Count -gt 0) {
            ...
            Write-AppLog "Belarc Advisor is not currently installed" -Level INFO -Component $script:AppConfig.Name
            return $null
        }
    }
    catch {
...
function Install-BelarcAdvisor {
    <#
    .SYNOPSIS
        Download and install Belarc Advisor
    .PARAMETER VersionInfo
        Version information hashtable from Get-BelarcAdvisorLatestVersion
    .OUTPUTS
    ...
    try {
        Write-AppLog "Starting Belarc Advisor installation process..." -Level INFO -Component $script:AppConfig.Name
        
        # Create temporary directory
        $tempDir = New-TempDirectory -Prefix "BelarcAdvisor"
        $installerPath = Join-Path $tempDir $VersionInfo.FileName
        
        # Download installer
        Write-AppLog "Downloading Belarc Advisor installer..." -Level INFO -Component $script:AppConfig.Name
        ...
        if ($installSuccess) {
            ...
            $newVersion = Get-InstalledBelarcAdvisorVersion
            
            if ($newVersion) {
                Write-AppLog "Installation verified: Belarc Advisor v$newVersion is now installed" -Level SUCCESS -Component $script:AppConfig.Name
                return $true
            }
            else {
...
function Test-InstallationRequired {
    ...
    $installedVersion = Get-InstalledBelarcAdvisorVersion
    
    if (-not $installedVersion) {
        Write-AppLog "Installation required: Belarc Advisor is not installed" -Level INFO -Component $script:AppConfig.Name
        return $true
    }
    ...
    # For Belarc Advisor, if we can't determine exact version, assume update is needed
...
try {
    Write-AppLog "=== Belarc Advisor Installation Script v1.0.0 ===" -Level INFO -Component $script:AppConfig.Name
    ...
    if ($Version) {
        ...
        $versionInfo = Get-BelarcAdvisorLatestVersion
    }
    ...
    $installResult = Install-BelarcAdvisor -VersionInfo $versionInfo
    
    if ($installResult) {
        Write-AppLog "Belarc Advisor installation completed successfully!" -Level SUCCESS -Component $script:AppConfig.Name
        exit 0
    }
    else {
        Write-AppLog "Belarc Advisor installation failed!" -Level ERROR -Component $script:AppConfig.Name
        exit 1
    }
}
catch {
