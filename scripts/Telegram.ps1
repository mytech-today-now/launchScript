...
<#
.SYNOPSIS
    Silent installation script for Telegram Desktop
    
.DESCRIPTION
    This script automatically downloads and installs the latest stable version of
    Telegram Desktop from the official website. Telegram is a cloud-based instant
    messaging service with a focus on security and speed.
    
.PARAMETER Force
    Force reinstallation even if Telegram is already installed
...
.NOTES
    ...
    Download Page: https://desktop.telegram.org/
#>
...
$script:AppConfig = @{
    ...
    DownloadPage = "https://desktop.telegram.org/"
    VersionCheckUrl = "https://api.github.com/repos/telegramdesktop/tdesktop/releases/latest"
    GitHubRepo = "telegramdesktop/tdesktop"
    DirectDownloadUrl = "https://telegram.org/dl/desktop/win64"
...
function Get-TelegramLatestVersion {
    <#
    .SYNOPSIS
        Get the latest stable version of Telegram from GitHub releases or direct download
    .OUTPUTS
        Hashtable containing version information and download URLs
    #>
    try {
        Write-AppLog "Fetching latest Telegram version information..." -Level INFO -Component $script:AppConfig.Name
        ...
        try {
            ...
            Write-AppLog "Could not get version from GitHub, using direct download" -Level WARN -Component $script:AppConfig.Name
            $version = "Latest"
        }
        ...
        $fileName = "TelegramSetup-$arch.exe"
        ...
        $versionInfo = @{
            ...
            Architecture = $arch
            ReleaseDate = if ($releaseInfo) { $releaseInfo.PublishedAt } else { (Get-Date).ToString() }
        }
...
function Get-InstalledTelegramVersion {
    <#
    .SYNOPSIS
        Get the currently installed version of Telegram
    .OUTPUTS
    ...
    try {
        ...
        if ($installedPrograms.Count -gt 0) {
            ...
            Write-AppLog "Telegram is not currently installed" -Level INFO -Component $script:AppConfig.Name
            return $null
        }
    }
    catch {
...
function Install-Telegram {
    ...
    .PARAMETER VersionInfo
        Version information hashtable from Get-TelegramLatestVersion
    .OUTPUTS
    ...
    try {
        Write-AppLog "Starting Telegram installation process..." -Level INFO -Component $script:AppConfig.Name
        
        # Create temporary directory
        $tempDir = New-TempDirectory -Prefix "Telegram"
        $installerPath = Join-Path $tempDir $VersionInfo.FileName
        
        # Download installer
        Write-AppLog "Downloading Telegram installer..." -Level INFO -Component $script:AppConfig.Name
        ...
        if ($installSuccess) {
            ...
            $newVersion = Get-InstalledTelegramVersion
            
            if ($newVersion) {
                Write-AppLog "Installation verified: Telegram v$newVersion is now installed" -Level SUCCESS -Component $script:AppConfig.Name
                return $true
            }
            else {
...
function Test-InstallationRequired {
    ...
    $installedVersion = Get-InstalledTelegramVersion
    
    if (-not $installedVersion) {
        Write-AppLog "Installation required: Telegram is not installed" -Level INFO -Component $script:AppConfig.Name
        return $true
    }
    ...
    # For Telegram, if we can't determine exact version, assume update is needed
...
try {
    Write-AppLog "=== Telegram Desktop Installation Script v1.0.0 ===" -Level INFO -Component $script:AppConfig.Name
    ...
    if ($Version) {
        ...
        $versionInfo = Get-TelegramLatestVersion
    }
    ...
    $installResult = Install-Telegram -VersionInfo $versionInfo
    
    if ($installResult) {
        Write-AppLog "Telegram installation completed successfully!" -Level SUCCESS -Component $script:AppConfig.Name
        exit 0
    }
    else {
        Write-AppLog "Telegram installation failed!" -Level ERROR -Component $script:AppConfig.Name
        exit 1
    }
}
catch {
