...
<#
.SYNOPSIS
    Silent installation script for Audacity
    
.DESCRIPTION
    This script automatically downloads and installs the latest stable version of
    Audacity from the official website. Audacity is a free, open-source, cross-platform
    audio software for multi-track recording and editing.
    
.PARAMETER Force
    Force reinstallation even if Audacity is already installed
...
$script:AppConfig = @{
    ...
    DownloadPage = "https://www.audacityteam.org/download/"
    VersionCheckUrl = "https://api.github.com/repos/audacity/audacity/releases/latest"
...
function Get-AudacityLatestVersion {
    <#
    .SYNOPSIS
        Get the latest stable version of Audacity from GitHub releases
    .OUTPUTS
        Hashtable containing version information and download URLs
    #>
    try {
        Write-AppLog "Fetching latest Audacity version information..." -Level INFO -Component $script:AppConfig.Name
        
        # Get latest release from GitHub API
        $releaseInfo = Get-LatestVersionFromGitHub -Repository $script:AppConfig.GitHubRepo
        
        # Parse version number (remove 'Audacity-' prefix and 'v' prefix if present)
        $version = $releaseInfo.Version -replace '^(Audacity-|v)', ''
        ...
        $installerAsset = $releaseInfo.Assets | Where-Object {
            $_.name -match "audacity-win-.*\.exe$" -and 
            ($arch -eq 'x64' -and $_.name -match "64bit|x64" -or 
             $arch -eq 'x86' -and $_.name -match "32bit|x86")
        } | Select-Object -First 1
        
        if (-not $installerAsset) {
            # Fallback: try to find any Windows installer
            $installerAsset = $releaseInfo.Assets | Where-Object {
                $_.name -match "audacity.*win.*\.exe$"
            } | Select-Object -First 1
        }
...
function Get-InstalledAudacityVersion {
    <#
    .SYNOPSIS
        Get the currently installed version of Audacity
    .OUTPUTS
    ...
    try {
        ...
        if ($installedPrograms.Count -gt 0) {
            ...
            Write-AppLog "Audacity is not currently installed" -Level INFO -Component $script:AppConfig.Name
            return $null
        }
    }
    catch {
...
function Install-Audacity {
    ...
    .PARAMETER VersionInfo
        Version information hashtable from Get-AudacityLatestVersion
    .OUTPUTS
    ...
    try {
        Write-AppLog "Starting Audacity installation process..." -Level INFO -Component $script:AppConfig.Name
        
        # Create temporary directory
        $tempDir = New-TempDirectory -Prefix "Audacity"
        $installerPath = Join-Path $tempDir $VersionInfo.FileName
        
        # Download installer
        Write-AppLog "Downloading Audacity installer..." -Level INFO -Component $script:AppConfig.Name
        ...
        if ($installSuccess) {
            ...
            $newVersion = Get-InstalledAudacityVersion
            
            if ($newVersion) {
                Write-AppLog "Installation verified: Audacity v$newVersion is now installed" -Level SUCCESS -Component $script:AppConfig.Name
                return $true
            }
            else {
...
function Test-InstallationRequired {
    ...
    $installedVersion = Get-InstalledAudacityVersion
    
    if (-not $installedVersion) {
        Write-AppLog "Installation required: Audacity is not installed" -Level INFO -Component $script:AppConfig.Name
        return $true
    }
...
try {
    Write-AppLog "=== Audacity Installation Script v1.0.0 ===" -Level INFO -Component $script:AppConfig.Name
    ...
    if ($Version) {
        ...
        $versionInfo = Get-AudacityLatestVersion
    }
    ...
    $installResult = Install-Audacity -VersionInfo $versionInfo
    
    if ($installResult) {
        Write-AppLog "Audacity installation completed successfully!" -Level SUCCESS -Component $script:AppConfig.Name
        exit 0
    }
    else {
        Write-AppLog "Audacity installation failed!" -Level ERROR -Component $script:AppConfig.Name
        exit 1
    }
}
catch {
